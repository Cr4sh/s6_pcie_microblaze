...
PVOID RuntimeGetSysInf(SYSTEM_INFORMATION_CLASS InfoClass)
{
    ...
    ULONG RetSize = 0, DataSize = 0x100;
    PVOID Info = NULL;

    while (true) 
    {    
        // allocate memory for data
        if ((Info = I_ExAllocatePool(NonPagedPool, DataSize)) == NULL) 
        {
        ...
        ns = (NTSTATUS)I_ZwQuerySystemInformation(InfoClass, Info, DataSize, &RetSize);
    ...
    {
        // function fails
        RtDbgMsg(__FILE__, __LINE__, "ZwQuerySystemInformation() ERROR 0x%.8x\n", ns);
...
PVOID RuntimeGetModuleBase(char *ModuleName)
{
    ...
    if (Info)
    {
        ...
        if (NT_SUCCESS(ns))
        {
            for (ULONG i = 0; i < Info->NumberOfModules; i += 1)
            {
                ...
                if (NT_SUCCESS(ns))
                {
                        ...
                        (BOOLEAN)I_RtlEqualUnicodeString(&usModuleName, &usCommonHalName_2, TRUE))
                    {
                        ...
                        for (int i_m = 0; i_m < HAL_NAMES_NUM; i_m += 1)
                        {
                            UNICODE_STRING usHalName;
                            I_RtlInitUnicodeString(&usHalName, wcHalNames[i_m]);
                             ...
                             (BOOLEAN)I_RtlEqualUnicodeString(&usModuleName, &usCommonNtName_2, TRUE))
                    {
                        ...
                        for (int i_m = 0; i_m < NT_NAMES_NUM; i_m += 1)
                        {
                            UNICODE_STRING usNtName;
                            I_RtlInitUnicodeString(&usNtName, wcNtNames[i_m]);
...
PVOID RuntimeGetProcAddress(PVOID Image, char *lpszFunctionName)
{
    ...
    else if (pImageExportDirectory->AddressOfNames != 0 && pImageExportDirectory->AddressOfNameOrdinals != 0)
    {
        ...
        for (ULONG i = 0; i < pImageExportDirectory->NumberOfNames; i += 1)
        {
            ...
            if (!std_strcmp(lpszName, lpszFunctionName))
            {
